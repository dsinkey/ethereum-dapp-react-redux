...
function setUIBasedOnNodeType() {
	...
	if (nodeType === 'metamask' || nodeType == 'testrpc') {
		setData(
			'lock_unlock_result',
			'Unlock / lock ( ) not supported for ' + nodeType,
			true
		);
	} else {
	...
	if (nodeType === 'geth' || nodeType === 'metamask') {
		...
		setData(
			'list_of_compilers',
			'getCompilers ( ) & compileSolidity ( ) not supported for ' + nodeType,
			true
		);
...
function copyBytecodeInterfaceToUI() {
	...
	document.getElementById(
		'compiled_abidefinition'
	).value = contract_abidefinition;
}
...
function generateTransactionJSON() {
	...
	setData(
		'send_transaction_object_json',
		JSON.stringify(tobject, undefined, 2),
		false
	);
}
...
function setEtherscanIoLink(aId, type, hashOrNumber) {
	...
	if (hashOrNumber) etherscanLinkA.innerHTML = 'etherscan.io';
	else etherscanLinkA.innerHTML = '';
}
